...
# Gegenstueck zu Frontend_Install_Remote.sh, das den Build frisch von
# GitHub laedt:
#
#   Frontend_Install_Remote.sh   braucht Internet auf dem MiSTer, holt
#                                 immer die neueste Fassung von GitHub
#   Frontend_Install_Offline.sh  braucht KEIN Internet, nimmt genau
#                                 die Dateien aus diesem Paket (dieser
#                                 Ordner)
#
...
#
# Ohne Rueckfragen:
#     ./Frontend_Install_Offline.sh --yes                # Autostart an, Overlay aus
#     ./Frontend_Install_Offline.sh --yes --stream       # zusaetzlich Overlay an
#     ./Frontend_Install_Offline.sh --yes --no-autostart # ohne Autostart
#
...
# "install_offline.sh" - jetzt umbenannt, siehe Kopfkommentar in
...
# Frontend_Install_Offline.sh daneben - daran laesst er sich zuverlaessig
# vom bereits INSTALLIERTEN Frontend unterscheiden (/media/fat/frontend
# hat kein Frontend_Install_Offline.sh daneben liegen, nur die
# Programmdateien selbst). Erkennt zusaetzlich noch den alten Dateinamen
...
if [ -z "$SRC" ]; then
    ...
    echo "Ordner mit 'frontend/frontend.py' UND 'Frontend_Install_Offline.sh' darin)."
    echo
...
# BUGFIX (siehe Frontend_Install.sh fuer die ausfuehrliche Begruendung):
...
if [ "$WANT_AUTOSTART" -eq 1 ]; then
    ...
    say "  Manuell starten: OSD -> Scripts -> Frontend_Start"
fi
...
if [ -f "$STARTUP" ] && grep -qF "f4_hotkey.sh" "$STARTUP"; then
    grep -v "f4_hotkey.sh" "$STARTUP" > "$STARTUP.tmp" 2>/dev/null \
        && mv "$STARTUP.tmp" "$STARTUP"
    echo "Alten F4-Eintrag aus user-startup.sh entfernt."
fi
rm -f "$FRONTEND_DIR/f4_hotkey.py" "$FRONTEND_DIR/f4_hotkey.sh" "$FRONTEND_DIR/f4_hotkey"
...
if [ "$WANT_STREAM" -eq 1 ]; then
    ...
    say "  Nicht aktiviert."
    say "  Spaeter einschalten: OSD -> Scripts -> Frontend_Stream_Toggle"
fi
...
if [ "$WANT_AUTOSTART" -eq 1 ]; then
    ...
    say "Naechster Schritt: OSD -> Scripts -> Frontend_Start"
fi
